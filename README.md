...
12. Переделать механизм распознавания «свой-чужой» между фронтом и беком с JSESSIONID на JWT. 
    Из сложностей – тебе придётся переделать отправку форм с фронта, чтоб добавлять хедер аутентификации.

Что не работает в проекте:

1. Авторизация oauth2 через сервер nginx. Ошибка 400: invalid_request
2. Регистрация нового аккаунта через форму на главной странице. Нового пользователя можно зарегистрировать
   только вручную, добавив запись в таблицу users в БД
3. Локализацию можно менять только на главной странице с английского на русский, 
   через кнопки, расположенные там же.
4. Проект некорректно работает в браузере Microsoft Edge - через сервер nginx отображается
   только содержание страницы в текстовом формате, статика загружается если обратиться к приложению напрямую.
